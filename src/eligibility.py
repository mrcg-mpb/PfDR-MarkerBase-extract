...
CRITERIA = (
    ...
    "PASSES. CHECK EVERY TABLE AND FIGURE before deciding — a paper often gives country-level "
    "totals in one table and the sub-national breakdown in another; base the judgement on the "
    "FINEST resolution found ANYWHERE in the paper, never on a single table in isolation. If finer "
    "locations are likely in supplementary material, flag the supplement rather than failing this "
    "criterion.\n"
    ...
    "breakdown the paper reports, not the overall span. A stated collection year — or a range of at "
    "most three years — IS sufficient resolution: e.g. 'conducted in 1995' bounds sampling to that "
    "single year (<=3y) and PASSES; do NOT mark it inconclusive merely because explicit start/end "
    "dates are absent. Fail on temporal grounds ONLY when the paper gives no usable collection time "
    "at all, or a single frequency pools samples across MORE than three years.\n"
    ...
    "supplement provided). Evidence for a criterion is frequently split across multiple tables, so "
    "never conclude a criterion is unmet from one table alone before checking the others. "
